# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 17:28:58 2015
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for x in element.attrib:
            if x in CREATED:
                ...
                node['pos'][1] = float(element.attrib[x])
            elif x == 'lat':
                node['pos'][0] = float(element.attrib[x])
            else:
        ...
        for x in element:
            if 'k' in x.attrib and not problemchars.search(x.attrib['k']):
                if x.attrib['k'].split(':')[0] == 'addr' and len(x.attrib['k'].split(':'))>2:
                    pass
                elif x.attrib['k'].split(':')[0] == 'addr' and len(x.attrib['k'].split(':'))<3:
                    if 'address' in node:
